...
最后,明度阶会被**重新锚定**,让你的精确输入色恰好落在 solid 档(500)。这样无论品牌色偏亮还是偏暗,`steps[500]` 永远等于你给的那个颜色 ——「500 就是品牌色」这条令牌合约始终成立。锚定时两个端点(50/950)保持不动、上下两段各自仿射缩放,所以**单调性绝不会被破坏**,你交付的颜色在成品里逐字可见。

::: tip 需要旧行为?
0.1.x 是把种子丢到**按明度最接近**的一档,亮度偏暗的品牌色会落到 500 之外(例如 `#2264f2` → 600)。想保留这套行为,传 `seedPlacement: 'nearest'`。
:::
